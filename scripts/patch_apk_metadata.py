...
for old_u in [u295, u294, u293, u292, u291, u290, u280, u270, u260, u250, u240, u232, u231, u230, u220, u210, u100]:
    if old_u in manifest:
        new_manifest = manifest.replace(old_u, u296)
        assert len(new_manifest) == len(manifest), "Manifest length changed!"
        entries[manifest_info] = new_manifest
        print(f"Successfully replaced UTF-16LE version with '2.9.6' in AndroidManifest.xml")
        break
else:
    if u296 in manifest:
        print("Notice: '2.9.6' already in AndroidManifest.xml")
    else:
...
if app_config_info:
    ...
    new_cfg = cfg
    for v in [b'"2.9.5"', b'"2.9.4"', b'"2.9.3"', b'"2.9.2"', b'"2.9.1"', b'"2.9.0"', b'"2.8.0"', b'"2.7.0"', b'"2.6.0"', b'"2.5.0"', b'"2.4.0"', b'"2.3.2"', b'"2.3.1"', b'"2.3.0"', b'"2.2.0"', b'"2.1.0"', b'"1.1.4"', b'"1.0.0"']:
        new_cfg = new_cfg.replace(v, b'"2.9.6"')
    entries[app_config_info] = new_cfg
    print("Successfully updated version to '2.9.6' in assets/app.config")
...
with zipfile.ZipFile(apk_path, 'r') as z_check:
    ...
    assert u296 in m, "Verification failed: 2.9.6 not found in AndroidManifest.xml!"
