...
print(f"Patching metadata in {apk_path} to v2.3.0...")
...
if u220 in manifest:
    new_manifest = manifest.replace(u220, u230)
    assert len(new_manifest) == len(manifest), "Manifest length changed!"
    entries[manifest_info] = new_manifest
    print("Successfully replaced UTF-16LE '2.2.0' with '2.3.0' in AndroidManifest.xml")
elif u210 in manifest:
    new_manifest = manifest.replace(u210, u230)
    assert len(new_manifest) == len(manifest), "Manifest length changed!"
    entries[manifest_info] = new_manifest
    print("Successfully replaced UTF-16LE '2.1.0' with '2.3.0' in AndroidManifest.xml")
elif u100 in manifest:
    new_manifest = manifest.replace(u100, u230)
    assert len(new_manifest) == len(manifest), "Manifest length changed!"
    entries[manifest_info] = new_manifest
    print("Successfully replaced UTF-16LE '1.0.0' with '2.3.0' in AndroidManifest.xml")
elif u230 in manifest:
    print("Notice: '2.3.0' already in AndroidManifest.xml")
else:
...
if app_config_info:
    ...
    new_cfg = cfg.replace(b'"2.2.0"', b'"2.3.0"').replace(b'"2.1.0"', b'"2.3.0"').replace(b'"1.1.4"', b'"2.3.0"').replace(b'"1.0.0"', b'"2.3.0"')
    entries[app_config_info] = new_cfg
    print("Successfully updated version to '2.3.0' in assets/app.config")
...
with zipfile.ZipFile(apk_path, 'r') as z_check:
    ...
    assert u230 in m, "Verification failed: 2.3.0 not found in AndroidManifest.xml!"
