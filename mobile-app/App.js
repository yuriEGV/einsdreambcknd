...
const getRecordingsDir = () => {
  ...
  if (!base) return null;
  return base.endsWith('/') ? base + 'einsdream_recordings/' : base + '/einsdream_recordings/';
};
...
const loadLocalRecordings = async () => {
  try {
    ...
    const recs = await Promise.all(
      files
        .filter(f => f.endsWith('.m4a') || f.endsWith('.aac'))
        .sort((a, b) => b.localeCompare(a))
        .map(async (filename) => {
          const uri = dir + filename;
          const info = await FileSystem.getInfoAsync(uri, { size: true }).catch(() => ({ size: 0 }));
          ...
          return {
            ...
            uri,
            size: info.size || 0,
            createdAt: new Date(ts || Date.now()),
            eventType: parts[1] || 'grabacion',
          };
        })
    );
    return recs;
  } catch (e) {
    console.warn('loadLocalRecordings error:', e);
    return [];
  }
};
...
class ErrorBoundary extends React.Component {
  ...
  render() {
    if (this.state.hasError) {
      return (
        <SafeAreaView style={{ flex: 1, backgroundColor: '#0F1218', justifyContent: 'center', alignItems: 'center', padding: 24 }}>
          ...
          <TouchableOpacity
            style={{ backgroundColor: '#6366F1', paddingVertical: 14, paddingHorizontal: 28, borderRadius: 12 }}
...
function Player({ rec, onClose }) {
  ...
  const toggle = async () => {
    ...
    try {
      ...
      console.warn('Player toggle error:', e);
    }
  };
  ...
  const seek = async (ms) => {
    if (!soundRef.current) return;
    try {
      const target = Math.max(0, Math.min(ms, dur || 1000));
  ...
  const label = eventLabel[rec.eventType] || rec.eventType;
    ...
    <View style={st.playerCard}>
      <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 14 }}>
        <View style={[st.badge, { backgroundColor: color + '22' }]}>
          <Text style={[st.badgeTxt, { color }]}>{label.toUpperCase()}</Text>
        </View>
      ...
      <Text style={st.playerTitle} numberOfLines={1}>{rec.filename}</Text>
      ...
      {err && <Text style={{ color: '#EF4444', textAlign: 'center', marginVertical: 10, fontSize: 13 }}>{err}</Text>}
      ...
      <TouchableOpacity
        ...
        onPress={(e) => {
          ...
          seek((e.nativeEvent.locationX / w) * dur);
        }}
      >
...
function MainApp() {
  ...
  useEffect(() => {
    monRef.current = isMonitoring;
  }, [isMonitoring]);

  useEffect(() => {
    if (isAuthenticated && tab === 'recordings') refresh();
  }, [isAuthenticated, tab]);
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      Alert.alert('Error de Sesión', e.response?.data?.message || 'Verifica tu conexión a internet');
    } finally {
  ...
  const handleLogout = () => {
    Alert.alert('Cerrar Sesión', '¿Deseas salir de Einsdream?', [
  ...
  const refresh = async () => {
    setRecsLoading(true);
    try {
      const items = await loadLocalRecordings();
      setRecs(items);
    } catch (e) {
      console.warn('Refresh recordings error:', e);
    } finally {
  ...
  const deleteRec = (item) => {
    Alert.alert('Eliminar Grabación', '¿Deseas eliminar ' + item.filename + '?', [
      { text: 'Cancelar', style: 'cancel' },
      {
        text: 'Eliminar',
        style: 'destructive',
        onPress: async () => {
          ...
          const dir = getRecordingsDir();
          if (dir) {
            await FileSystem.deleteAsync(dir + item.filename, { idempotent: true }).catch(() => {});
          }
          refresh();
        },
      },
  ...
  const startMonitoring = async () => {
    try {
      ...
      if (p.status !== 'granted') {
        Alert.alert('Permiso Requerido', 'Se necesita permiso de micrófono para monitorear el sueño.');
        return;
      }
      await Audio.setAudioModeAsync({
      ...
      Alert.alert('Error', 'No se pudo iniciar el monitor: ' + e.message);
    ...
    try {
      ...
      r.setOnRecordingStatusUpdate((s) => {
        if (s.metering !== undefined) {
          const d = Math.round(Math.min(95, Math.max(30, 95 + s.metering)));
          setDb(d);
      ...
      try {
        if (recording) {
          ...
          if (uri) {
            ...
            const ts = Date.now();
            const fname = `einsdream_${type}_${ts}.m4a`;
            const dir = getRecordingsDir();
            if (dir) {
              await FileSystem.copyAsync({ from: uri, to: dir + fname }).catch(() => {});
            }
            const meta = {
              eventType: type,
      ...
      <SafeAreaView style={st.container}>
        ...
        <ScrollView contentContainerStyle={{ flexGrow: 1, justifyContent: 'center' }} keyboardShouldPersistTaps="handled">
          <View style={{ flex: 1, justifyContent: 'center', padding: 28 }}>
            ...
            <Text style={st.appSub}>Monitor Acústico Nocturno</Text>
            <View style={st.authBox}>
              ...
              <TouchableOpacity style={[st.loginBtn, loginLoading && { opacity: 0.7 }]} onPress={handleLogin} disabled={loginLoading} activeOpacity={0.85}>
                {loginLoading ? <ActivityIndicator color="#fff" /> : <Text style={st.loginBtnTxt}>INICIAR SESIÓN</Text>}
              </TouchableOpacity>
            </View>
            <Text style={{ color: '#475569', fontSize: 13, textAlign: 'center', marginTop: 20, lineHeight: 18 }}>
              Para soporte contacte a su médico o al equipo Einsdream.
            </Text>
          </View>
        </ScrollView>
      </SafeAreaView>
  ...
  const MonitorTab = () => (
    <ScrollView contentContainerStyle={st.tabPad}>
      <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
        <View>
          <Text style={st.brand}>EINSDREAM</Text>
          <Text style={{ fontSize: 12, color: '#64748B', marginTop: 2 }}>Monitor Nocturno</Text>
        </View>
      ...
      <TouchableOpacity style={[st.bigBtn, isMonitoring ? st.bigBtnOn : st.bigBtnOff]} onPress={isMonitoring ? stopMonitoring : startMonitoring} activeOpacity={0.85}>
        ...
        <Text style={{ color: '#F8FAFC', fontWeight: '800', fontSize: 16, textAlign: 'center', letterSpacing: 1 }}>
          {isMonitoring ? 'DETENER\nMONITOREO' : 'INICIAR\nMONITOREO'}
        </Text>
      </TouchableOpacity>

      <View style={st.metricsRow}>
        <View style={st.metric}>
          <Text style={st.metLbl}>TIEMPO</Text>
          <Text style={st.metVal}>{formatTime(sessionSecs)}</Text>
        </View>
        <View style={st.metDiv} />
        <View style={st.metric}>
          <Text style={st.metLbl}>NIVEL</Text>
          <Text style={[st.metVal, { color: db > 52 ? '#F59E0B' : '#10B981' }]}>{isMonitoring ? db + ' dB' : '-- dB'}</Text>
        </View>
        <View style={st.metDiv} />
        <View style={st.metric}>
          <Text style={st.metLbl}>EVENTOS</Text>
          <Text style={[st.metVal, { color: '#818CF8' }]}>{events}</Text>
        </View>
      </View>

      {latestEvt && (
        <View style={st.evtCard}>
          <Text style={{ color: '#818CF8', fontWeight: '700', fontSize: 14, marginBottom: 4 }}>
            {(eventLabel[latestEvt.eventType] || latestEvt.eventType).toUpperCase()} ({latestEvt.confidence}%)
          </Text>
          <Text style={{ color: '#64748B', fontSize: 12 }}>
            {new Date(latestEvt.detectedAt).toLocaleTimeString()} · {latestEvt.intensityDb} dB
          </Text>
      ...
      <View style={st.statusBox}>
        <Text style={{ color: '#64748B', fontSize: 12, fontWeight: '700', letterSpacing: 0.8, marginBottom: 10, textTransform: 'uppercase' }}>
          Estado
        </Text>
        <Text style={{ color: '#94A3B8', fontSize: 14, marginBottom: 6 }}>
          🎙️ Micrófono: <Text style={{ color: '#10B981', fontWeight: '700' }}>ACTIVO</Text>
        </Text>
        <Text style={{ color: '#94A3B8', fontSize: 14 }}>
          🔋 Modo Nocturno: <Text style={{ color: '#10B981', fontWeight: '700' }}>Optimizado</Text>
        </Text>
      </View>
      <Text style={{ color: '#334155', fontSize: 12, textAlign: 'center', lineHeight: 18, marginTop: 10 }}>
        Las grabaciones se guardan localmente en tu celular. Ve a la pestaña Grabaciones para escucharlas.
      </Text>
    </ScrollView>
  );
  ...
  const RecordingsTab = () => (
    <View style={[st.tabPad, { flex: 1 }]}>
      ...
      <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 4 }}>
        ...
        <TouchableOpacity onPress={refresh} style={st.refreshBtn}>
          <Text style={{ color: '#818CF8', fontSize: 13, fontWeight: '600' }}>Actualizar</Text>
        </TouchableOpacity>
      </View>
      ...
      {recsLoading ? (
        ...
        <View style={{ alignItems: 'center', paddingTop: 40 }}>
          ...
          <Text style={{ color: '#475569', fontSize: 14, textAlign: 'center', lineHeight: 22, paddingHorizontal: 20 }}>
            Las grabaciones aparecerán aquí cuando el monitor detecte sonidos nocturnos. Inicie el monitor en la pestaña Monitor.
          </Text>
        </View>
      ) : (
        <FlatList
          ...
          renderItem={({ item }) => {
            ...
            return (
              <TouchableOpacity
                style={[st.recItem, active && { borderColor: '#6366F1', backgroundColor: 'rgba(99,102,241,0.08)' }]}
                onPress={() => setSelRec(item)}
                activeOpacity={0.8}
              >
                <View style={[st.recIcon, { backgroundColor: col + '22' }]}>
                  <Text style={{ fontSize: 22 }}>🎵</Text>
                </View>
                <View style={{ flex: 1 }}>
                  <Text style={{ color: col, fontSize: 12, fontWeight: '800', letterSpacing: 0.8, marginBottom: 2 }}>
                    {(eventLabel[item.eventType] || item.eventType).toUpperCase()}
                  </Text>
  ...
  const ProfileTab = () => (
    <ScrollView contentContainerStyle={st.tabPad}>
      ...
      <View style={[st.card, { marginBottom: 14 }]}>
        {[['Cuenta', user?.role === 'admin' ? 'Administrador' : 'Paciente'], ['Grabaciones locales', recs.length.toString()], ['Versión', 'v2.1.0']].map(([k, v]) => (
      ...
      <View style={[st.card, { marginBottom: 20 }]}>
        <Text style={{ color: '#818CF8', fontSize: 14, fontWeight: '700', marginBottom: 8 }}>¿Dónde están mis grabaciones?</Text>
        <Text style={{ color: '#64748B', fontSize: 14, lineHeight: 22 }}>
          Las grabaciones se guardan de forma segura en este dispositivo. Puede escucharlas en la pestaña Grabaciones sin necesidad de internet.
          También se sincronizan con la plataforma web Einsdream automáticamente.
        </Text>
      </View>
      <TouchableOpacity style={st.logoutBtn} onPress={handleLogout} activeOpacity={0.85}>
        <Text style={{ color: '#F87171', fontSize: 16, fontWeight: '700' }}>Cerrar Sesión</Text>
      </TouchableOpacity>
    </ScrollView>
  );
  ...
  return (
    <SafeAreaView style={st.container}>
      ...
      <View style={st.tabBar}>
        {[
          { id: 'monitor', icon: isMonitoring ? '🔴' : '🌙', label: 'Monitor' },
          { id: 'recordings', icon: '🎧', label: 'Grabaciones' },
          { id: 'profile', icon: '👤', label: 'Perfil' },
        ].map(t => (
          <TouchableOpacity
            key={t.id}
            style={[st.tabItem, tab === t.id && { backgroundColor: 'rgba(99,102,241,0.12)' }]}
            onPress={() => {
              setTab(t.id);
              if (t.id === 'recordings') refresh();
            }}
            activeOpacity={0.8}
          >
            <Text style={{ fontSize: 22, marginBottom: 3 }}>{t.icon}</Text>
            <Text style={[{ color: '#475569', fontSize: 11, fontWeight: '600' }, tab === t.id && { color: '#818CF8' }]}>
              {t.label}
            </Text>
          </TouchableOpacity>
        ))}
